...
*Landed.* Two decisions worth recording. **A plugin reaches the compiler as a `library_manifests[]` entry** with an empty `wit` and its WASM hash in `compiletime_wasm_sha256` — there is no other slot for a dependency's own manifest, and a new top-level key would be `RQD002`. This is what makes a rebuilt plugin change the request document, which Phase 5's cache key depends on. **`[paths].owns` entries are refused if they escape the project**, lexically: a dependency that can name `../..` as a folder it owns pulls arbitrary files off the developer's disk into the compilation.
...
*Landed, with one correction to the spec's key.* **The key is `sha256(request_sha256 + compiler_version)`, not the request hash alone.** The request document describes the compilation's *input*; CMP-06 promises identical output only for identical input **through the same compiler**. Keyed on the request alone, an upgraded toolchain reads and overwrites the entries of the one it replaced, and the developer silently gets a component built by a compiler they are no longer using. This was not theoretical — it was caught by four CLI tests that shared a request shape and got served each other's artifacts.

Two consequences worth knowing. **The cache stores the compiler's raw response tarball**, not a re-serialized `CompileArtifact`: a hit and a miss then run the same `from_tar` over the same bytes, so byte-identical reproduction is structural rather than something to keep true by hand. And **every failure mode degrades to compiling** — an unreadable entry, an unwritable cache, a compiler that will not report its version. A cache exists to make a build faster; one that can fail a build is a liability.
...
1. **How does `.cln/lock.toml` handle plugins vs. libraries?** Manager §00.3.2 says `cln add` is overloaded for both. The lockfile schema isn't in `foundation/schema/` yet (I checked). Framework needs to know whether a lock entry is a library (Clean source, needs compile) or a plugin (pre-built WASM, gets loaded as-is). Proposal: lockfile entries carry an explicit `kind = "library" | "plugin"` field. **Still unconfirmed with Manager.** Phase 2 shipped against the proposal rather than blocking on it, with the reader isolated in `framework-core::lockfile` so a different answer is a one-file change; an entry without `kind` is refused (`CFG002`), never guessed. Confirm when the Manager side is reachable.
